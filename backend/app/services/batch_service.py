...
from app.db.models import Customer, Payment, Subscription, RecoveryCase, BatchRun, RecoveryPolicy
...
def run_demo_batch(db: Session, batch_size: int = 500) -> BatchRun:
    ...
    Ingests events, runs the full agent state machine loop, and calculates all metrics directly from DB records.
    """
    ...
    # 1. Generate or fetch synthetic customers & payments for batch
    batch_ref = f"BATCH-{uuid.uuid4().hex[:8].upper()}"
    
    # We create distinct scenarios within the batch to guarantee coverage of Scenarios A, B, C, D, E:
    ...
    for pay in created_payments:
        try:
            ...
            if case.is_escalated or case.status == "STOPPED":
    ...
    # 3. Compute exact metrics FROM DB RECORDS
    cases_query = db.query(RecoveryCase).all()
    total_events = len(cases_query)
    
    rev_at_risk = sum(c.amount_at_risk for c in cases_query)
    rev_recovered = sum(c.recovered_amount for c in cases_query if c.status == "RECOVERED")
    recovery_rate = round((rev_recovered / max(1.0, rev_at_risk)) * 100.0, 2)
    
    recovered_cases = [c for c in cases_query if c.status == "RECOVERED"]
    escalated_cases = [c for c in cases_query if c.status == "ESCALATED"]
    stopped_cases = [c for c in cases_query if c.status == "STOPPED"]
    ...
    batch_run = BatchRun(
        ...
        unnecessary_intervention_rate=round((len(stopped_cases) + len(escalated_cases)) / max(1, total_events) * 100.0, 2),
